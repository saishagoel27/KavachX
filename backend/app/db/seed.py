...
``python -m scripts.seed`` (and therefore ``make seed`` and the Docker entrypoint) and the startup
provisioner in :mod:`app.db.provision`. One implementation, so the manual and automatic paths
cannot drift apart.
"""
...
class SeedError(RuntimeError):
    """A precondition outside the database is missing, so the seed cannot complete."""
...
async def demo_tenant_present() -> bool:
    """True when the demo user already exists — i.e. the seed has nothing to add."""
